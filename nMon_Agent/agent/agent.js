...
function spooky(response, paramMap, callback, socket) {
                  ...
                  //Page 내에 Javascript 오류 발생시 호출
                    ...
                    this.on("timeout", function() {
                        ...
                        this.on('load.started.custom', function() {
                            ...
                            path = "/image/FindElement/"   + year + "/" + mon + "/" + day + "/" + hour;
                            har_path = "/har/FindElement/" + year + "/" + mon + "/" + day + "/" + hour;
                        ...
                        this.on('complete', function() {
                        	...
                        	var uu_id = uuid.v4();
                            var file      = path + "/" + uu_id + ".jpg";
                            this.waitFor(function(){this.emit('save.image', file, uu_id);return true;});
                            ...
                            var log = {type : "complete", msg : "COMPLETE", image : file, load : resultMap.loadtime,
                                           run : resultMap.runtime};
                            console.log(JSON.stringify(log));
                            this.waitFor(function() {
                                this.emit('callback', resultMap, uploadList);
                                return true;
                            });
                            this.waitFor(function() {
                            	this.exit();
...
//                        });
                        ...
                        this.on('save.imageElement', function (element, file){
                        	this.waitFor(function(){
                                this.evaluate(function(){
                                        document.body.bgColor = 'white';
                                });
                            ...
                            this.waitFor(function(){
                                try{
                                    this.captureSelector(LogPath + "/temp/" + file, element);
                                    uploadList.push({
                                        "filename" : file,
                                        "ori_path" : LogPath + "/temp/",
                                        "new_path" : path + "/"
                        ...
                        this.on('popup.created', function (newPage) {
                            var log = {type : "popup", msg : "Created popup !! "};
                            console.log(JSON.stringify(log));
                        });

                        this.waitFor(function() {
                            var login;
                            if (typeof paramMap.login != 'undefined') {
                            ...
                            this.waitFor(function() {
                                ...
                                if (typeof data.url != 'undefined') {
                                    ...
                                    if (!this.exists(element)) {
                                        ...
                                        var file_image = uu_id + ".jpg";
                                        var log = {type : "find", msg : "FIND ELEMENT : " + element, 
                                                   element : element, image : path + "/" + uu_id + ".jpg"};
                                        this.waitFor(function(){this.emit('save.imageElement', element, file_image);return true;});
                                        console.log(JSON.stringify(log));
                                    }
                                    ...
                                    this.waitFor(function() {
                                        try {
                                            eval(data.custom_js);
                                            var log = {type : "info", msg : "Custom JS 실행."};
                                            console.log(JSON.stringify(log));
                                            return true;
                                        } catch (e) {
    ...
    spooky.on('console', function (line) {
        //console.log(line);
    	if(line.slice(0,1)=="{"){
            socket.emit('msg_user', paramMap.scid, "agent", line);
        } else {
        	console.log(line);
        }
    });

    spooky.on('callback', function(resultMap, uploadList) {
        try {
            callback(response, resultMap, uploadList);
        } catch (e) {
